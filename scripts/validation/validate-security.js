...
const [
  ...
  read("src/server/security/runtime-policy.js"),
  read("database/bootstrap/app-role.sql"),
  read(".github/workflows/ci.yml"),
  read("scripts/validation/validate-compose-integration.sh"),
  read("public/app/course-app.js"),
  read("src/server/api/handler.js"),
  read("src/server/persistence/migrate.js"),
  read("database/migrations/002_session_client_capability.sql"),
  read("public/workers/python-runner-worker.js"),
  read("scripts/database/init-secrets.sh"),
...
check(
  ...
  "The production image must copy the explicit public tree and exclude private solutions."
);
...
check(
  composeIntegrationGate.includes("scripts/database/init-secrets.sh") &&
