...
test("trusted exercise manifest follows every repository chapter and exercise order", async () => {
  ...
  vm.runInContext(
    await readFile(resolve(REPOSITORY_ROOT, "public/content/exercise-data.js"), "utf8"),
    context,
  );
  ...
  assert.equal(
    getExerciseFile("py13-level-status").relativePath,
    "Py13 Pygame Game Lab/ex07.py"
  );
...
test("submission mirror rejects traversal, symlink escapes, and solution directories", async (context) => {
  ...
  for (const protectedDirectory of [
    resolve(REPOSITORY_ROOT, "curriculum/solutions/Py01 First Programs"),
    resolve(REPOSITORY_ROOT, "public/assets"),
  ]) {
